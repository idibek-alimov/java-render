...
public interface OrderRepository extends CrudRepository<Order,Long> {
    ...
    @Query(value = "SELECT COUNT(id),SUM(original_price*quantity)as price,EXTRACT(HOUR FROM CAST(created_at as time)) as time FROM item WHERE DATE(created_at)=CURRENT_DATE AND owner_id=?1 AND status=1 GROUP BY time ORDER BY time",nativeQuery = true)
    List<DayOrderStatisticDTO> findTodayOrdersShipping(Long id);

    @Query(value = "SELECT COUNT(id),SUM(original_price*quantity)as price,EXTRACT(DAY FROM DATE(created_at))as day " +
    ...
    @Query(value = "SELECT COUNT(id),SUM(original_price*quantity)as price,EXTRACT(HOUR FROM CAST(created_at as time)) as time FROM item WHERE DATE(created_at)=CURRENT_DATE AND owner_id=?1 AND status=3 GROUP BY time ORDER BY time",nativeQuery = true)
